...
namespace Sincerely_Human
{
    class Program
    {
        ...
        public static Menu humanMenu, humanSettings;
        ...
        {
            Chat.Print("Sincerely Human Loaded!", Color.HotPink);
            Chat.Print("Made by Sakebi", Color.White);
            Chat.Print("Script humanly or you will get banned in a week!", Color.HotPink);

            humanMenu = MainMenu.AddMenu("Sincerely Human", "Sincerely Human");
            humanSettings = humanMenu.AddSubMenu("Settings", "settings");
            humanSettings.AddGroupLabel("Settings");
            humanSettings.AddLabel("Delays");
            humanSettings.Add("Spells", new CheckBox("Humanize Spells"));
            humanSettings.Add("Attacks", new CheckBox("Humanize Attacks"));
            humanSettings.Add("Movements", new CheckBox("Humanize Movements"));
            humanSettings.Add("MinClicks", new Slider("Minimum clicks per second", _random.Next(6, 7), 1, 7));
            humanSettings.Add("MaxClicks",
                new Slider("Maximum clicks per second",
        ...
        {
            Chat.Print("<font color='#ff0000'>Failed to load Sincerely Human!</font>");
            humanMenu = MainMenu.AddMenu("Sincerely Human (Disabled)", "Sincerely Human (Disabled)", "Sincerely Human (Disabled)");
            humanMenu.AddLabel("Failed to load Sincerely Human! Please wait for update.");
        }
        ...
        public static void Main(string[] args)
        {
            ...
            Drawing.OnDraw += onDrawArgs =>
            {
                ...
                Drawing.DrawText(Drawing.Width - 190, 100, Color.HotPink, "Blocked : " + BlockedCount + " Clicks");
            };
        ...
        public static void Player_OnIssueOrder(Obj_AI_Base sender, PlayerIssueOrderEventArgs issueOrderEventArgs)
        {
            if (sender.IsMe && !issueOrderEventArgs.IsAttackMove)
            {
                if (issueOrderEventArgs.Order == GameObjectOrder.AttackUnit ||
                    ...
                    !humanMenu["Attacks"].Cast<CheckBox>().CurrentValue)
                    return;
                if (issueOrderEventArgs.Order == GameObjectOrder.MoveTo &&
                    !humanMenu["Movements"].Cast<CheckBox>().CurrentValue)
                    return;
            }
            ...
            if (Environment.TickCount - order.Value <
                Randomize(
                    1000 / humanMenu["MaxClicks"].Cast<Slider>().CurrentValue,
                    1000 / humanMenu["MinClicks"].Cast<Slider>().CurrentValue) + _random.Next(-10, 10))
            {
                BlockedCount += 1;
        ...
        {
            if (!humanSettings["Spells"].Cast<CheckBox>().CurrentValue)
                return;
